...
- [Design Details](#design-details)
  ...
  - [Examples](#examples)
    - [Sharable DeviceClass's selector](#sharable-deviceclasss-selector)
...
- [Alternatives](#alternatives)
  - [Selecting/Deselecting Sharable Devices](#selectingdeselecting-sharable-devices)
  - [Preventing Same Sharable Device from Being Allocated Multiple Times in the Same Claim](#preventing-same-sharable-device-from-being-allocated-multiple-times-in-the-same-claim)
...
- a capacity-aware scheduling mechanism that allows limiting or guaranteeing the sharable device capacity,
...
- a distinct attribute constraint to prevent allocating the same sharable device in the same claim multiple times.

Relations to other KEPs:
- [KEP 4815](https://github.com/kubernetes/enhancements/issues/4815): The partitioned devices can be a shareble device or have mutually exclusive partitions where one partition is sharable and the other is not.
...
A motivating use case is to allocate a sharable network device in the [CNI DRA driver](https://github.com/kubernetes-sigs/cni-dra-driver)
...
A DRA driver for networks advertises sharable devices for two interfaces eth1 and eth2
which each connect to the same virtual LAN, 
the admin makes those device available through a DeviceClass selecting only sharable devices,
and users request access through a request which references that DeviceClass.

#### Story 2

When requesting two interfaces, the user requests two devices. To ensure that they don't end up with the same sharable device for each request, they specify that the driver-specific "interfaceName" attribute must be different.
...
- The driver includes both sharable and unsharable devices. There is a risk that a user may be allocated a sharable device
  (e.g., a sharable network device) and accidentally configure it with the HostDevice CNI plugin.
  This would move the device from the host into the user's pod, preventing other users from accessing the sharable device.

  **Mitigation:**
  - Administrators should define clear device classes for sharable and unsharable devices to prevent such misallocations.
...
A sharable device can only be allocated once its consumability has been verified
...
type Device struct {
...
   // Sharable marks whether the device is sharable.
   //
   // A device with sharable="true" can be allocated more than once.
   //
   // +optional
...
// The sharing policy can be either specify a range of valid values or a discrete set of them.
// Each policy has a default value, either explicitly or implicitly.
...
type DeviceConstraint struct {
   ...
   //
   // This constraint is used to avoid allocating multiple requests to the same sharable device
...
type DeviceRequestAllocationResult struct {
  ...
   ...
   // ShareUID uniquely identifies the specific allocation result of the sharable device.
   // Set only when allocation is on a sharable device.
   //
   // +optional
   // +featureGate=DRAConsumableCapacity
   ShareUID *types.UID
  ...
  //
  // This field references only consumable capacities of a sharable device and is empty when there are none.
  //
  // +optional
  // +featureGate=DRAConsumableCapacity
...
- A new `device.sharable` key is introduced in the CEL selector,
  enabling policies and constraints to recognize whether a device supports sharable allocation.
...
- Once all selection and constraint checks pass, the allocation is valid. The allocation result is updated with:
  - The sharable allocation identifier, which uniquely identifies the allocation on a sharable device.
...
- Finally, the sharable allocation identifiers and consumed capacities from all internal results
  are propagated to the DeviceRequestAllocationResult.

### Examples

#### Sharable DeviceClass's selector

```yaml
selectors:
  - cel:
      expression: |-
        device.sharable == true
```
...
spec:
  devices:
    requests:
    - name: macvlan-1
      deviceClassName: simple-sharable.networking.x-k8s.io
...
- Extension of TestAllocator in `structured` module. 
  ...
  |Test case|Sharable|Device(s) Capacity|Allocated Capacity|Sharable Device Class|Claim request(s)|Expected success
  |---|---|---|---|---|---|---|
  |sharable-device-with-consumable-capacity|yes|2 consumable|0|yes|1+1|yes
  |sharable-device-with-exceeded-consumable-capacity-request|yes|2 consumable|0|yes|1+2|no
  |sharable-device-with-some-remaining-consumable-capacity|yes|2 consumable|1|yes|1|yes
  |sharable-device-with-no-available-consumable-capacity|yes|1 consumable|1|yes|1|no
  |sharable-device-with-unconsumable-capacity|yes|1 unconsumable|0|yes|1+1|yes
  |unsharable-device-with-single-consumable-capacity-request|no|1 consumable|0|yes|1|yes
  |unsharable-device-with-multiple-consumable-capacity-request|no|1 consumable|0|yes|1+1|no
  |exclude-unsharable-device-from-class-selector|yes|0|0|no|0|no
  |one-sharable-device-with-distinct-constraint|yes|2 consumable|0|yes|1+1, distinct|no
  |two-sharable-devices-with-distinct-constraint|yes|2x1 consumable|0|yes|1+1, distinct|yes
...
- `ListAllAllocatedCapacity` unit test to get AllocatedCapacityCollection without unsharable devices.
...
- Add test user story 1 to 6 in in `scheduler_perf/dra.go` when defining a sharable device with and without consumable capacity.
...
- Feature implemented behind feature a gate.
  Feature Gates is disabled by default.
...
During version skew, where the API server supports the feature but the scheduler does not, the introduced field can be set, but it will be ignored. In this case, all devices will be treated as non-sharable, and all capacities will be considered non-consumable. No errors or warnings will be triggered, but the field will have no effect.
...
work. They will also continue to work when restarted as devices that have been 
prepared and configured for them won't be modified across the restart.

The DRA driver itself should also be able to survive a rollback but will lose the 
ability to advertise new shareable resources.
...
This feature will not guarantee to keep the shared resources advertised before the 
feature has been disabled. DRA-Drivers will then have to monitor and maintain 
its `ResourceSlices`.
...
Multiple `ResoureClaim` will share the same resource of a `ResourceSlice`.

###### How can someone using this feature know that it is working for their instance?

- [x] API .status
  - Other field: `.status.allocation.devices.results[].sharedUID` and 
   `.status.allocation.devices.results[].consumedCapacities` will be set for 
    a `ResoureClaim` using the DRA feature.
...
This feature depends on the DRA structured parameters feature 
([KEP-4381 - DRA Structured Parameters](https://github.com/kubernetes/enhancements/issues/4381)).
A third-party DRA driver is also required to publish and prepare shared resources.
...
Yes as new fields are introduced in the `ResourceSlice` and `ResourceClaim` API objects.

###### Will enabling / using this feature result in increasing time taken by any operations covered by existing SLIs/SLOs?

The scheduler must now evaluate partial capacity availability for the resources
requested in the `ResourceClaims` making the scheduling logic more complex.
Devices for claims that use this feature will then be affected and may require
more time to get allocated.
...
- Initial proposal: 2025-01-23

## Drawbacks

This feature introduces more complexity to the scheduler.

## Alternatives

### Selecting/Deselecting Sharable Devices

**Current Approach:**

Extend the CEL selector to recognize device.sharable for filtering sharable devices.

**Alternative:**
Introduce explicit flags in the resource request:

- AllowShared: Opt-in to allow sharable devices.
- RequireShared: Only allow sharable devices.

(Default: sharable devices are excluded unless explicitly allowed.)

Pros:
- Does not affect unsharable device selection.
...
Cons:
- Adds complexity to the allocation logic for sharable devices.
...
### Preventing Same Sharable Device from Being Allocated Multiple Times in the Same Claim
...
**Alternative:**
The scheduler enforces this behavior implicitly—never allocate the same sharable device multiple times to the same resource claim.

Pros:
- Avoids any API changes—logic handled internally.

Cons:
- Doesn’t support cases where a pod legitimately needs multiple fractions of capacity from the same sharable device.
